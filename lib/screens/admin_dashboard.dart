...
class _AdminDashboardState extends State<AdminDashboard> {
        ...
        Row(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Tạo dữ liệu thật (Demo)'),
  ...
  Future<void> _seedData() async {
    try {
      ...
      // 1. Seed Curriculum KTPM
      await firestore.collection('curriculums').doc('Kỹ thuật Phần mềm').set({
        'major': 'Kỹ thuật Phần mềm',
        'categories': [
          {
            'title': 'Khối kiến thức đại cương',
            'isRequired': true,
            'courses': [
              {'courseId': 'PHI 100', 'courseName': 'Phương Pháp Luận', 'credits': 2, 'status': 'Đã hoàn tất (A)'},
              {'courseId': 'CS 201', 'courseName': 'Tin Học Ứng Dụng', 'credits': 3, 'status': 'Đã hoàn tất (B+)'},
            ]
          },
          {
            'title': 'Khối kiến thức cơ sở ngành',
            'isRequired': true,
            'courses': [
              {'courseId': 'CS 211', 'courseName': 'Lập Trình Cơ Sở', 'credits': 3, 'status': 'Đã hoàn tất (A-)'},
              {'courseId': 'CMU-CS 303', 'courseName': 'Fundamentals of Computing 1', 'credits': 3, 'status': 'Đang học'},
            ]
          }
        ]
      });

      // 1.5 Seed Curriculum HTTTQL
      await firestore.collection('curriculums').doc('Hệ thống Thông tin Quản lý').set({
        'major': 'Hệ thống Thông tin Quản lý',
        'categories': [
          {
            'title': 'Khối kiến thức Kinh tế',
            'isRequired': true,
            'courses': [
              {'courseId': 'ECO 151', 'courseName': 'Kinh tế Vi Mô', 'credits': 3, 'status': 'Đã hoàn tất (B)'},
              {'courseId': 'MGT 201', 'courseName': 'Quản trị học', 'credits': 3, 'status': 'Đang học'},
            ]
          },
          {
            'title': 'Khối Hệ thống Thông tin',
            'isRequired': true,
            'courses': [
              {'courseId': 'CMU-IS 100', 'courseName': 'Intro to Information Systems', 'credits': 3, 'status': 'Đã hoàn tất (A)'},
              {'courseId': 'IS 220', 'courseName': 'Cơ sở dữ liệu', 'credits': 3, 'status': 'Chưa học'},
            ]
          }
        ]
      });

      // 2. Seed Student Grades
      await firestore.collection('student_grades').doc('29201122422').set({
        'studentId': '29201122422',
        'semesters': [
          {
            'semesterName': 'Học Kỳ I - Năm Học 2023-2024',
            'courses': [
              {'courseId': 'CMU-IS 100', 'classId': 'CMU-IS 100 EIS', 'type': 'LEC', 'courseName': 'Introduction to Information Systems', 'credits': 3, 'grade10': 7.30, 'gradeChar': 'B', 'grade4': 3.00},
              {'courseId': 'CS 211', 'classId': 'CS 211 AQIS', 'type': 'LEC', 'courseName': 'Lập Trình Cơ Sở', 'credits': 3, 'grade10': 8.20, 'gradeChar': 'A-', 'grade4': 3.65},
            ],
            'summary': {
              'totalCredits': 6,
              'avg10': 7.75,
          ...
          {
            'semesterName': 'Học Kỳ II - Năm Học 2023-2024',
            'courses': [
              {'courseId': 'CMU-CS 303', 'classId': 'CMU-CS 303 FIS', 'type': 'LEC', 'courseName': 'Fundamentals of Computing 1', 'credits': 3, 'grade10': 4.50, 'gradeChar': 'C-', 'grade4': 1.65},
            ],
            'summary': {
              'totalCredits': 3,
              'avg10': 4.50,
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tạo dữ liệu mẫu lên Firestore thành công!'), backgroundColor: Colors.green));
    } catch (e) {
      Navigator.pop(context); // close dialog
  ...
  Widget _buildGrading() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          Icon(Icons.assignment, color: AppColors.adminColor, size: 28),
          SizedBox(width: 12),
          Text('CHẤM BÀI', style: TextStyle(color: AppColors.adminColor, fontSize: 18, fontWeight: FontWeight.bold)),
        ]),
        const SizedBox(height: 24),
